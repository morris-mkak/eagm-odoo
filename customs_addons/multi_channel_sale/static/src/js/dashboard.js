odoo.define('multi_channel_sale.backend.dashboard',function (require) {
	'use strict'
	...
	var MultichannelDashboard = AbstractAction.extend({
		...
		on_action (e) {
			...
			switch (action) {
				case 'import':
					return this.do_action('multi_channel_sale.open_import_wizard_action')
				case 'export':
					return this.do_action('multi_channel_sale.open_export_wizard_action')
				case 'open':
		...
		on_action (e) {
			...
			if (action && instance)
				switch (action) {
					...
					case 'evaluate':
						return this.do_action('multi_channel_sale.action_feed_sync', {
							additional_context: {
								default_channel_id: instance,
							},
						})
