...
class FP_core:
    ...
    def __analyzeResponse(self, res_root):
        ...
        for ppp in range(0, length):
            ...
            elif typ == "Decimal_with_format" and value != "":
                result_obj.append(float(value))
            elif typ == "Decimal_plus_80h" and value != "":
                result_obj.append(float(value))
            elif typ == "Status":
                if value == "1":
    ...
    def __checkVersion(self, res_root):
        ...
        try:
            ...
            if defv > self._timestamp:
                raise ServerException("Server definitions are newer than client library.",
                                      SErrorType.ServerDefsMismatch)
            elif defv < self._timestamp:
                raise ServerException("Server definitions are older than client library.",
                                      SErrorType.ServerDefsMismatch)
            self.__ok = True
        except ServerException as fpe:
            raise fpe
