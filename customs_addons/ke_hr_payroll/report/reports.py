            ...
            if rec.slip_ids:
                ...
                for key, slip in enumerate(rec.slip_ids):
                    ...
                    tiers_m['t1'] = slip.line_ids.search(
                        [('salary_rule_id', '=',
                          rec.env.ref('ke_hr_payroll.ke_rule50').id),
                         ('slip_id', '=', slip.id)],
                        limit=1).total  # NSSF Employee contributions
                    # Employer Contributions
                    tiers_e['t1'] = slip.line_ids.search(
                        [('salary_rule_id', '=',
                          rec.env.ref('ke_hr_payroll.ke_rule60').id),
                         ('slip_id', '=', slip.id)],
                        limit=1).total  # NSSF Employer contributions
                    # Dermine the TOTALS
                    total_income += income
                    ...
                    tiers_m = {x: tiers_m[x] for x in tiers_m.keys() if (
                            tiers_m[x] or tiers_e[
                        x])}  # Ignore Zero Contributions from Member
                    ...
                    for k in sorted(tiers_m):
                        ...
                        # SURNAME
                        ws['B' + str(fr + key + t)
                           ] = slip.employee_id.display_name.split(' ')[
                                   -1] or ''
                        # OTHER NAMES
                        ws['C' + str(fr + key + t)
        ...
        for rec in self:
            if rec.slip_ids:
                ...
                for k, x in enumerate(
                        ...
                        0:26]):  # 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
                    ws[x + str(fr - 1)] = cols[k]
                ...
                for key, slip in enumerate(rec.slip_ids):
                    ...
                    cross_taxable = slip.line_ids.search(
                        [('salary_rule_id', '=',
                          rec.env.ref('ke_hr_payroll.ke_rule45').id),
                         ('slip_id', '=', slip.id)],
                    ...
                    deds = slip.line_ids.search(
                        [
                            ('salary_rule_id',
                             '=',
                             rec.env.ref('ke_hr_payroll.ke_rule115').id),
                            ('slip_id',
                             '=',
                             slip.id)],
            ...
            for slip in rec.slip_ids:
                if slip.contract_id.car and slip.contract_id.cars:
                    for car in slip.contract_id.cars:
                        data_cars = [
                            ...
                            slip.employee_id.disability and 'C_Disabled_Employees_Dtls' or 'B_Employees_Dtls',
                            # B
                            car.name,  # C
                            car.make,  # D
                            car.body in [
                                'saloon'] and 'Saloon Hatch Backs and Estates' or car.body in [
                                'pickup'] and 'Pick Ups, Panel Vans Uncovered' or car.body in [
                                'cruiser'] and 'Land Rovers/ Cruisers(excludes Range Rovers and vehicles of similar nature)',
                            # E
                            car.cc_rate,  # F
                            car.cost_type,  # G
                            car.cost_type in [
                ...
                if not slip.employee_id.disability:
                    ...
                    data_employee = [
                        ...
                        slip.employee_id.resident and 'Resident' or 'Non-Resident',
                        # C
                        slip.employee_id.emp_type in [
                            'primary'] and 'Primary Employee' or 'Secondary Employee',
                        # D
                        slip.contract_id.wage or 0.0,  # E
                        slip.line_ids.search([('salary_rule_id',
                                               '=',
                                               rec.env.ref(
                                                   'ke_hr_payroll.ke_rule17').id),
                                              ('slip_id',
                                               '=',
                                               slip.id)],
                        ...
                        slip.line_ids.search(
                            [('salary_rule_id', '=', rec.env.ref(
                                'ke_hr_payroll.ke_rule15').id),
                             ('slip_id', '=', slip.id)], limit=1).total or 0.0,
                        # H Leave Pay
                        slip.line_ids.search([('salary_rule_id',
                                               '=',
                                               rec.env.ref(
                                                   'ke_hr_payroll.ke_rule13').id),
                                              ('slip_id',
                                               '=',
                                               slip.id)],
                                             limit=1).total or 0.0,
                        ...
                        slip.line_ids.search(
                            [
                                ('salary_rule_id',
                                 '=',
                                 rec.env.ref('ke_hr_payroll.ke_rule37').id),
                                ('slip_id',
                                 '=',
                                 slip.id)],
                            limit=1).total or 0.0,
                        ...
                        (not slip.contract_id.house and str(
                            slip.employee_id.global_income)) or
                        (
                                    slip.contract_id.house and slip.contract_id.house_type == 'director' and str(
                                slip.employee_id.global_income)) or
                        None,
                        ...
                        (not slip.contract_id.house and "Benefit not given"),
                        # R
                        (
                                    slip.contract_id.house and slip.contract_id.house_type not in [
                                'director'] and slip.contract_id.rent) or
                        (
                                    slip.contract_id.house and slip.contract_id.house_type in [
                                'director'] and '0.0') or
                        ...
                        (
                                    slip.contract_id.house and slip.contract_id.house_type not in [
                                'director'] and slip.contract_id.rent_recovered) or
                        (
                                    slip.contract_id.house and slip.contract_id.house_type in [
                                'director'] and '0.0') or
                        ...
                        (slip.line_ids.search([('salary_rule_id', '=',
                                                rec.env.ref(
                                                    'ke_hr_payroll.ke_rule55').id),
                                               ('slip_id', '=', slip.id)],
                                              limit=1).total + (
                                 slip.employee_id.pension and slip.employee_id.pen_contrib or 0.0)) if slip.employee_id.resident and slip.employee_id.emp_type in [
                            'primary'] else None,
                        # Y Actual Pension Contributions including NSSF
                        ...
                        (
                                slip.line_ids.search(
                                    [
                                        ('salary_rule_id',
                                         '=',
                                         rec.env.ref('ke_hr_payroll.ke_rule73').id),
                                        ('slip_id',
                                         '=',
                                         slip.id)],
                                    limit=1).total or 0.0) if slip.employee_id.emp_type in [
                            'primary'] else None,
                        # AA
                        (
                                slip.line_ids.search(
                                    [
                                        ('salary_rule_id',
                                         '=',
                                         rec.env.ref('ke_hr_payroll.ke_rule71').id),
                                        ('slip_id',
                                         '=',
                                         slip.id)],
                                    limit=1).total or 0.0) if slip.employee_id.emp_type in [
                        ...
                        (
                                slip.line_ids.search(
                                    [
                                        ('salary_rule_id',
                                         '=',
                                         rec.env.ref('ke_hr_payroll.ke_rule91').id),
                                        ('slip_id',
                                         '=',
                                         slip.id)],
                                    limit=1).total or 0.0) if slip.employee_id.resident and slip.employee_id.emp_type in [
                            'primary'] else None,
                        # AF Personal Tax relief
                        (slip.line_ids.search([('salary_rule_id',
                                                '=',
                                                rec.env.ref(
                                                    'ke_hr_payroll.ke_rule96').id),
                                               ('slip_id',
                                                '=',
                                                slip.id)],
                        ...
                        # AG Insurance Relief
                        None,
                        # AH PAYE Tax after deduct relief & Insurance Relief - Computed
                        slip.line_ids.search([('salary_rule_id', '=',
                                               rec.env.ref(
                                                   'ke_hr_payroll.ke_rule101').id), (
                                                  'slip_id', '=', slip.id)],
                                             limit=1).total or 0.0
                        # AI Self Assesed PAYE Tax
                    ]
                ...
                if slip.employee_id.disability:
                    data_disabled = [
                        ...
                        slip.employee_id.emp_type in [
                            'primary'] and 'Primary Employee' or 'Secondary Employee',
                        # D
                        slip.employee_id.disability_cert,
                        # E Excemption Cert Number
                        ...
                        (slip.line_ids.search([('salary_rule_id', '=',
                                               ...
                                               ('slip_id', '=', slip.id)],
                                              limit=1).total + (
                                 slip.employee_id.pension and slip.employee_id.pen_contrib or 0.0)) or 0.0,
                        # Z Actual Contributions (NSSF + Pension/Provident)
                        None,  # AA Permissible Limit (20,000 Ksh)
                        slip.line_ids.search([('salary_rule_id',
                                               '=',
                                               rec.env.ref(
                                                   'ke_hr_payroll.ke_rule73').id),
                                              ('slip_id',
                                               '=',
                                               slip.id)],
                                             limit=1).total or 0.0,
                        ...
                        # AI Insurance Relief
                        None,
                        # AJ PAYE Tax after deduct personal relief & Insurance Relief
                        slip.line_ids.search([('salary_rule_id', '=',
                                               rec.env.ref(
                                                   'ke_hr_payroll.ke_rule101').id), (
                                                  'slip_id', '=', slip.id)],
                                             limit=1).total or 0.0
                        # AK Self Assesed PAYE Tax
                    ]
